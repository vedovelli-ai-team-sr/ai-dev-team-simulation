...
// SVG icon components (extracted to module level for performance optimization)
const taskIcon = (
...
/**
 * Unified notification configuration combining icon, label, and styling.
 * Extracted to module level for performance optimization - prevents recreation on every render.
 *
 * This approach is preferred over separate maps because:
 * - Single source of truth: all properties for a notification type are co-located
 * - Type-safe: TypeScript enforces all types have entries and all properties present
 * - Extensible: Adding a new notification type requires one entry, not four updates
 *
 * Note: Some types intentionally share icons:
 * - task_assigned/unassigned both use taskIcon (same entity, different action)
 * - sprint_started/completed both use sprintIcon (same entity, different lifecycle)
 * Visual distinction comes from the badge color and label text.
 */
 ...
 *
 * Architecture:
 * Uses a consolidated `notificationConfig` object instead of separate maps.
 * This is preferred because all notification properties are co-located, reducing
 * the cognitive load and making it harder to miss updates when extending types.
 */
      ...
      <div className="flex gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 mb-1">
            <span
              className={`inline-block px-2 py-0.5 rounded-full text-xs font-medium ${config.badgeColor}`}
            >
              {config.label}
            </span>
            {!notification.read && (
