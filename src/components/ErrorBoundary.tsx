...
> {
  constructor(props: ErrorBoundaryProps) {
    super(props)
    this.state = { error: null, hasError: false, fallbackError: false }
  }

  static getDerivedStateFromError(error: Error): Omit<ErrorBoundaryState, 'fallbackError'> {
  ...
  retry = () => {
    this.setState({ error: null, hasError: false, fallbackError: false })
  }

  render() {
    // If the fallback itself threw an error, show emergency fallback
    if (this.state.fallbackError) {
      return (
        <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded">
          <h2 className="font-bold text-lg mb-2">Critical Error</h2>
          <p className="mb-4 text-sm">An error occurred while rendering the error UI.</p>
          <button
            onClick={this.retry}
    ...
    if (this.state.hasError && this.state.error) {
      try {
        ...
        // If fallback throws, render emergency fallback
        console.error('Fallback component threw an error:', fallbackError)
